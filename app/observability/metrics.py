...
GATEWAY_OVERHEAD = Histogram(
    "llm_gateway_overhead_seconds", "Gateway overhead logic execution time in seconds", ["team_id", "logical_tier"]
)
...
CIRCUIT_STATE = Gauge(
    "llm_gateway_circuit_breaker_state", "Circuit breaker state status (0=closed, 1=half-open, 2=open)", ["provider"]
)
...
TEAM_SPEND = Counter("llm_gateway_spend_usd_total", "Total spend in USD", ["team_id", "team_name", "provider"])

# 7. Token throughput count (Counter)
TOKEN_COUNT = Counter("llm_gateway_tokens_total", "Total tokens processed", ["team_id", "provider", "token_type"])
