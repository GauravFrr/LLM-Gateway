...
class ChatCompletionRequest(BaseModel):
    ...
    messages: List[Message] = Field(
        ..., min_items=1, description="The list of messages for chat completion."
    )
    stream: bool = Field(
        False, description="Whether to stream the response chunks."
    )
    max_tokens: Optional[int] = Field(
        None, gt=0, description="The maximum number of tokens to generate."
    )
...
class ModelAccessRequest(BaseModel):
    ...
    fallback_provider: Optional[Literal["gemini", "claude", "groq", "ollama"]] = Field(None)
