        ...
        raise HTTPException(
            ...
            detail={
                "error": {
                    ...
                    "message": f"Team name '{body.name}' already exists."
                }
            }
        )
...
async def create_or_update_model_access(
    id: uuid.UUID,
    ...
    if not team:
        raise HTTPException(
            ...
            detail={
                "error": {
                    "code": "invalid_request",
                    "message": f"Team with ID '{id}' not found."
                }
            }
        )
    ...
    result = await db.execute(
        select(TeamModelAccess).where(
            TeamModelAccess.team_id == id,
            TeamModelAccess.logical_tier == body.logical_tier
        )
    )
